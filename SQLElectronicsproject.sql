...
CALL GetPriceByPercentile(0.26, @q1);   
CALL GetPriceByPercentile(0.55, @q2);  
CALL GetPriceByPercentile(0.73, @q3) ; 
select @q1 as Q1,@q2 as Median , @q3 as Q3  ;

SELECT * FROM  ELECTRONICSDATA
 WHERE PRICE <(@q1-1.5*(@q3-@q1)) OR PRICE >(@q3+1.5*(@q3-@q1));  -- OUTLIERS  ON positive side 
 
 -- CREATE DATA AS PER BUCKETS
...
-- COVRIANCE 
	SELECT 
        ...
        (reviews_count - (SELECT AVG(reviews_count) FROM electronicsdata ))) / (COUNT(*) - 1)),2) AS covariance
...
--  COTINGENCY TABLE (COUNT OF CATEGORY EACH COMPANY IS DEALING IN)
 SELECT BRANDNAME , 
...
GROUP BY BRANDNAME;
 ...
 --  GROUPED SUMMARY STATISTICS Grouped Summary Statistics (Mean, Variance, Standard Deviation)
SELECT bRANDNAME, 
...
GROUP BY BRANDNAME;          
 ...
 -- COVRIANCE across different categories
